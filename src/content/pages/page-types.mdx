...
<Callout>
**New** — Posts are the newest addition to Loomwork.
</Callout>

Posts are a separate content collection for date-driven content: blogs, news, updates, changelogs. Unlike the templates above, posts live in their own directory (`src/content/posts/`) and have their own routing at `/posts/`.
...
**Layout:** Uses the `default` template via `Content.astro`.
**Listing:** Automatic reverse-chronological listing at `/posts/` with cards showing title, date, reading time, and tags.
**Routing:** `/posts/{slug}` for individual posts.
**Drafts:** Set `draft: true` to hide a post from the listing and prevent its page from being generated.

**Example:** [Posts listing](/posts/) · [Full posts documentation](/posts-guide/)
...
| `posts` | — | 52rem | ✓ | Blogs, news, updates |
